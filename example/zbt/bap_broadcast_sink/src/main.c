...
static size_t g_ply_stream_idx;
            ...
            for (uint8_t i = 0U; i < frames_blocks_per_sdu; i++)
            {
                for (uint16_t j = 0U; j < frames_per_block; j++)
                {
                    ...
                    if (stream_index == g_ply_stream_idx)
                    {
                        ...
                        //if (num_of_sample != 160)
                        //{
                        //printk("num_of_sample=%d\n", num_of_sample);
                        //RT_ASSERT(0);
                        //}
...
static int cmd(int argc, char *argv[])
{
    if (argc > 1)
    {
        // set channel which can send to speak
        if (strcmp(argv[1], "set_chnl") == 0)
        {
            g_ply_stream_idx = atoi(argv[2]);
        }
        else if (strcmp(argv[1], "set_vol") == 0)
        {
            // set local volume that shall be smaller than 15
            audio_server_set_private_volume(AUDIO_TYPE_BT_MUSIC, atoi(argv[2]));
        }
    }
